...
# 가중치별 간선 내림차순 정렬
# itemgetter(0) => (weight, slot, elot) 중 weight로 정렬
...
print('중복 간선 목록 정렬 =>', end=' ')
print(newAry)

# 가중치 높은 간선부터 제거
index = 0
# 0:춘천, 1:서울, 2:속초, 3:대전, 4:광주, 5:부산
while len(newAry) > (SIZE-1): # 간선의 수가 정점 수 (정점 수 - 1) 될때 까지 반복
    ...
    startYN = findVertex(G, start) # 서울에 연결된 선이 있는 지 확인
    endYN = findVertex(G, end) # 광주에 열결된 간선 확인

    if startYN and endYN: # 둘다 다른 간선 있으니까 지금 간선을 지워도 됨
        del(newAry[index])
    else: # 연결된 간선이 없으니까 지웠던 인접행렬 값을 원상 복귀
